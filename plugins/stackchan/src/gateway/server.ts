...
export function scheduleFlush(state: StackChanState, promptId: string, deviceId: string): void {
...
export async function startGateway(
  ...
  if (hostTts?.synthesize) {
    ...
    ctx.logger.warn(
      "stackchan: host TTS unavailable; outbound will use plugin config or fall back to text-only",
    );
  }
  ...
  switch (message.type) {
    ...
    default:
      send(socket, {
        type: "error",
        code: "unsupported",
        message: `not implemented: ${(message as DeviceClientMessage).type}`,
      });
  }
}
...
export function handleBinaryFrame(state: StackChanState, socket: WebSocket, buf: Buffer): void {
