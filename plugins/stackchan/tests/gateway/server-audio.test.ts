...
describe('handleAudioMessage', () => {
  it('audio.start → audio.ready and audio.end → deliver + transcript.final', async () => {
    ...
    await handleAudioMessage(ctx as unknown as GatewayContext<StackChanState>, reg, sock, { type: 'audio.start', promptId: 'p1' })
    ...
    await handleAudioMessage(ctx as unknown as GatewayContext<StackChanState>, reg, sock, { type: 'audio.end', promptId: 'p1' })
    const frames = (sock as unknown as ReturnType<typeof fakeSocket>).sent.map((s: string) => JSON.parse(s))
